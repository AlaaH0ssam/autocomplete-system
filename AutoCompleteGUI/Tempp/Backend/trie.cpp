...
void trie::load_from_file(const string& Dictionary) {
    ...
    while (file >> word >> freq) {
        if (validator.InputValidator(word)) {
            insert(word, freq);
            global_frequency_map[validator.to_lower_case(word)] = freq;
        }
        else {
...
bool trie::deleteWordGUI(const std::string& word) {
    std::string deletedWord = validator.to_lower_case(word);
    std::vector<std::string> allWords = dfs("");
